...
public class RunDatabaseHelper extends SQLiteOpenHelper {
	...
	public long insertRun(Run run) {
		...
		cv.put(COLUMN_RUN_START_DATE, run.startDate.getTime());
	...
	public static class RunCursor extends CursorWrapper {
		...
		public Run getRun() {
			if (isBeforeFirst() || isAfterLast())
				return null;
			Run run = new Run();
			run.id = getLong(getColumnIndex(COLUMN_RUN_ID));
			run.startDate = new Date(getLong(getColumnIndex(COLUMN_RUN_START_DATE)));
			return run;
		}
	}
